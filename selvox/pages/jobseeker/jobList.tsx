...
const JobListPage = () => {
  ...
  useEffect(() => {
    const fetchJobListings = async () => {
      try {
        const response = await fetch(`https://localhost:7095/api/JobListing`);
        if (!response.ok) {
          throw new Error(
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {jobListings.map((listing) => (
                ...
                >
                  ...
                  <p className="text-gray-600 mb-2">{listing.jobType}</p>
                  <p className="text-gray-600 mb-2">{listing.salaryRange}</p>
