...
export default function UsersData() {
    ...
    useEffect(() => {
        fetchUsers(userPage, pageSize);
        fetchJobListings(jobListingPage, pageSize);
    }, [userPage, jobListingPage]);

    const fetchUsers = async (page, pageSize) => {
        try {
            const res = await fetch(`https://localhost:7095/api/Admin/usersData`);
    ...
    const fetchJobListings = async (page, pageSize) => {
        try {
            const res = await fetch(`https://localhost:7095/api/Admin/joblistings?page=${page}&pageSize=${pageSize}`);
            const data = await res.json();
            setJobListings(data);
        } catch (error) {
